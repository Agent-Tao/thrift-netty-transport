    ...
    public synchronized void reload()
    {
        try {
            ...
            if (trustCertificateModified || clientCertificateModified || privateKeyModified) {
                ...
                if (clientCertificatesFileWatch.isPresent()) {
                    //certificateChain = readCertificateChain(clientCertificatesFileWatch.get().getFile()).toArray(new X509Certificate[0]);
                }
                ...
                /*X509Certificate[] trustChain = readCertificateChain(trustCertificatesFileWatch.getFile()).toArray(new X509Certificate[0]);
