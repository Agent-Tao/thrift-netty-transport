        ...
        extends ByteToMessageDecoder
{
    ...
    private final ThriftServerProcessor thriftServerProcessor;
    private final Integer maxFrameSize;
    private final boolean assumeClientsSupportOutOfOrderResponses;

    public ThriftProtocolDetection(ThriftServerProcessor thriftServerProcessor, Integer maxFrameSize, boolean assumeClientsSupportOutOfOrderResponses)
    {
        this.maxFrameSize = requireNonNull(maxFrameSize, "maxFrameSize is null");
        this.thriftServerProcessor = requireNonNull(thriftServerProcessor, "thriftServerProcessor is null");
    ...
    protected void decode(ChannelHandlerContext context, ByteBuf in, List<Object> out)
    {
        ...
        if ((magic & UNFRAMED_MESSAGE_MASK) == UNFRAMED_MESSAGE_FLAG) {
            ...
            switchToTransport(context, Transport.UNFRAMED, protocol);
            return;
        }
        ...
        if ((magic2 & HEADER_MAGIC_MASK) == HEADER_MAGIC) {
            switchToTransport(context, Transport.HEADER, Optional.empty());
            return;
        }
        ...
        switchToTransport(context, Transport.FRAMED, protocol);
    }
    ...
    private void switchToTransport(ChannelHandlerContext context, Transport transport, Optional<Protocol> protocol)
    {
        ...
        pipeline.addLast(thriftServerProcessor);
